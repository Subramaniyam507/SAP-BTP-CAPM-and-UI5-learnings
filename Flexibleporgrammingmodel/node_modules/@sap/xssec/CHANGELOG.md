...
## 4.11.0 - 2025-10-23
- add new method `IdentityServiceSecurityContext#getIdToken()` that can be used to get an id token that can be forwarded to reuse services. The method returns the security context's jwt directly if it is suitable or, when the security context was created from an App-to-App access token, exchanges and caches it for an id token. The id token cache is configurable via the `idTokenCache` property of the IdentityService configuration (default: `{"enabled": true, "size": 100}`). Alternatively, a custom cache implementation that implements a standard `get(key)`, `set(key, value)` cache signature can be provided via the `idTokenCache.impl` property.
- add experimental `XsuaaLegacyExtension` class for legacy applications migrating from XSUAA to IAS authentication and optionally AMS authorization. It can be instantiated and passed to the `context.extensions` array property of the IdentityService configuration. See JsDoc for details and contact maintainers before building production functionality with it until the API is no longer declared as @experimental.
